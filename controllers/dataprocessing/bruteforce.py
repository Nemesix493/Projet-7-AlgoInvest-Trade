...
class Bruteforce(BestActionsListInterface):
    ...
    def get_best_actions_list(cls, actions_list: pd.DataFrame, invest_max: float = 500) -> pd.DataFrame:
        invest_actions_list = cls.building_invest_actions_list(
            pd.DataFrame({'price': []}),
            actions_list,
            invest_max
        )
        return max(invest_actions_list, key=calc.action_list_profit)
